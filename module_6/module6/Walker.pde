public class Walker
{
  ...
  public void checkEdges()
  {
    ...
    if (this._position.x >= Window.right)
    {
      _position.x = Window.right; // safeguard our position to not overflow toward the farther end of the edge
      _velocity.x *= -1;
    }
    
    if (this._position.y <= Window.bottom)
    {
      _position.y = Window.bottom;
      _velocity.y *= -1;
    }
  }
